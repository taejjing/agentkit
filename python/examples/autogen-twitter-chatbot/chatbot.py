...
def initialize_agent(config: CdpEvmWalletProviderConfig):
    ...
    #     host=f"{os.getenv('OLLAMA_HOST')}:{os.getenv('OLLAMA_PORT')}",
    # )
...
# Autonomous Mode
async def run_autonomous_mode(agent_executor: AssistantAgent, interval=5):
    ...
    while True:
        try:
            ...
            async for chunk in agent_executor.run_stream(
                task=TextMessage(content=thought, source="User")
            ):
                if isinstance(chunk, BaseChatMessage): #and chunk.source == "assistant":
                    print("\n-------------------")
                    print(chunk.to_text())
                    print("-------------------")
...
# Chat Mode
async def run_chat_mode(agent_executor: AssistantAgent):
